...
while ($Row = mysqli_fetch_assoc($result)){
	...
	$Suit = $Row["Suit"];
	@$subtitle .= $SuitMaster. ":". $Suit. " ";
	$SuitName = $SuitMaster == 1 ? $Suit : "";
}
...
$(document).ready(function() {
	chart = new Highcharts.Chart({
		...
		title: {
			text: 'Diving Suit and Air Consumption'
		},
		subtitle: {
			text: '<?=$subtitle?>'
		},
		...
		yAxis: {
			title: {
				text: 'Kind of Suit <?=$subtitle?>'
			}
		},
		tooltip: {
			formatter: function() {
					return ''+
					this.x +' lit/min, '+ this.y +' ';
			}
		},
		legend: {
			layout: 'vertical',
